...
heightsere, "eventually fall" means that the droplet will eventually be at a lower level if it moves in that direction. 
...
{
public:
	//Reference: https://leetcode.com/problems/pour-water/solution/
	vector<int> pourWater(vector<int>& heights, int V, int K)
	{
		while (V-- > 0)
		{
			...
			{
				auto i = K, best = K;
				while (0 <= i + d && i + d < heights.size() && heights[i + d] <= heights[i])
				{
